...
function businessAddress() {
  const addr = {
    ...
    country: 'US',
    // USPS requires the SENDER's email and phone on every shipment — an
    // address missing these gets silently rejected (the vague/empty-body
    // 429 we were chasing was actually this validation failure).
    email: process.env.BUSINESS_EMAIL,
    phone: process.env.BUSINESS_PHONE
  };
  // TEMPORARY debug log — shows exactly what's being sent to Shippo, so we
  // can see the real runtime values instead of guessing. Remove this line
  // once the label issue is confirmed fixed.
  console.log('businessAddress() built:', JSON.stringify(addr));
  return addr;
}
...
async function purchaseLabel({ direction, customerAddress, weightLb }) {
  ...
  if (transaction.status !== 'SUCCESS') {
    // TEMPORARY debug log — logged as its OWN line so Vercel's log viewer
    // doesn't collapse/truncate it the way it does when this same detail
    // is embedded inside an Error's stack trace. Remove once resolved.
    console.error('Shippo transaction.messages (full):', JSON.stringify(transaction.messages, null, 2));
